...
import com.vsm.business.service.custom.DelegateDocumentCustomService;
import com.vsm.business.web.rest.errors.BadRequestAlertException;
import com.vsm.business.service.custom.search.service.DelegateDocumentSearchService;
...
@RestController
@RequestMapping("/api")
public class DelegateDocumentCustomRest {

    private final Logger log = LoggerFactory.getLogger(DelegateDocumentCustomRest.class);
    ...
    private final DelegateDocumentCustomService delegateDocumentCustomService;

    private final DelegateDocumentSearchService delegateDocumentSearchService;

    public DelegateDocumentCustomRest(
        DelegateDocumentService delegateDocumentService,
        DelegateDocumentRepository delegateDocumentRepository,
        DelegateDocumentCustomService delegateDocumentCustomService,
        DelegateDocumentSearchService delegateDocumentSearchService
    ) {
        this.delegateDocumentService = delegateDocumentService;
        this.delegateDocumentRepository = delegateDocumentRepository;
        this.delegateDocumentCustomService = delegateDocumentCustomService;
        this.delegateDocumentSearchService = delegateDocumentSearchService;
    }
    ...
    @GetMapping("/_all/delegate-documents")
    public ResponseEntity<IResponseMessage> getAll(){
        log.info("DelegateDocumentCustomRest: getAll()");
        List<DelegateDocumentDTO> result = delegateDocumentCustomService.getAll();
     ...
     */
    @PostMapping("/search_custom/delegate-documents")
    public ResponseEntity<IResponseMessage> customSearch(@RequestBody DelegateDocumentDTO delegateDocumentDTO, Pageable pageable) throws IllegalAccessException {
        ISearchResponseDTO result = this.delegateDocumentSearchService.simpleQuerySearch(delegateDocumentDTO, pageable);
        log.debug("DelegateDocumentCustomRest: customSearch(IBaseSearchDTO: {}): {}", delegateDocumentDTO, result);
        return ResponseEntity.ok().body(new LoadedMessage(result));
    }

    @PostMapping("/v2/search_custom/delegate-documents")
    public ResponseEntity<IResponseMessage> customSearchWithParam(@RequestBody DelegateDocumentDTO delegateDocumentDTO, @RequestParam(required = false) Map<String, Object> paramMaps, Pageable pageable) throws IllegalAccessException {
        ISearchResponseDTO result = this.delegateDocumentSearchService.simpleQuerySearchWithParam(delegateDocumentDTO, paramMaps, pageable);
        log.debug("DelegateDocumentCustomRest: customSearchWithParam(IBaseSearchDTO: {}): {}", delegateDocumentDTO, result);
        return ResponseEntity.ok().body(new LoadedMessage(result));
    }
}
