...
public class ExceptionTranslator implements ProblemHandling, SecurityAdviceTrait {
    ...
    public ProblemBuilder prepare(final Throwable throwable, final StatusType status, final URI type) {
        ...
        if (activeProfiles.contains(JHipsterConstants.SPRING_PROFILE_PRODUCTION)) {
            ...
            if (containsPackageName(throwable.getMessage())) {
                return Problem
                    ...
                    // DuowngTora: thêm cho lỗi không có quyền \\
            if(throwable instanceof AuthorityException){
                return Problem
                    ...
                    .withDetail("NOT AUTHORITY !")
                    .withCause(
                        Optional.ofNullable(throwable.getCause()).filter(cause -> isCausalChainsEnabled()).map(this::toProblem).orElse(null)
                    );
            }

                    // DuowngTora: thêm cho lỗi ResponseStatusException \\
        ...
        return Problem
            ...
            .withTitle(status.getReasonPhrase())
            .withStatus(status)
            .withDetail(/*throwable.getMessage()*/ "Error!!!")
            .withCause(
                Optional.ofNullable(throwable.getCause()).filter(cause -> isCausalChainsEnabled()).map(this::toProblem).orElse(null)
            );
    }
    ...
    private boolean containsPackageName(String message) {
        ...
        return StringUtils.containsAny(message, "org.", "java.", "net.", "javax.", "com.", "io.", "de.", "com.vsm.business");
    }
}
