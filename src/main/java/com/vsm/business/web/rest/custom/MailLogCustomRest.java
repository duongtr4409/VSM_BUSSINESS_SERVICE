...
import com.vsm.business.service.custom.MailLogCustomService;
import com.vsm.business.web.rest.errors.BadRequestAlertException;
import com.vsm.business.service.custom.search.service.MailLogSearchService;
...
@RestController
@RequestMapping("/api")
public class MailLogCustomRest {

    private final Logger log = LoggerFactory.getLogger(MailLogCustomRest.class);
    ...
    private final MailLogCustomService mailLogCustomService;

    private final MailLogSearchService mailLogSearchService;

    public MailLogCustomRest(MailLogService mailLogService, MailLogRepository mailLogRepository, MailLogCustomService mailLogCustomService, MailLogSearchService mailLogSearchService) {
        this.mailLogService = mailLogService;
        this.mailLogRepository = mailLogRepository;
        this.mailLogCustomService = mailLogCustomService;
    ...
    @GetMapping("/_all/mail-logs")
    public ResponseEntity<IResponseMessage> getALl(){
        log.info("MailLogCustomRest: getAll()");
        List<MailLogDTO> result = this.mailLogCustomService.getAll();
     ...
     */
    @PostMapping("/search_custom/mail-logs")
    public ResponseEntity<IResponseMessage> customSearch(@RequestBody MailLogDTO mailLogDTO, Pageable pageable) throws IllegalAccessException {
        ISearchResponseDTO result = this.mailLogSearchService.simpleQuerySearch(mailLogDTO, pageable);
        log.debug("MailLogCustomRest: customSearch(IBaseSearchDTO: {}): {}", mailLogDTO, result);
        return ResponseEntity.ok().body(new LoadedMessage(result));
    }

    @PostMapping("/v2/search_custom/mail-logs")
    public ResponseEntity<IResponseMessage> customSearchWithParam(@RequestBody MailLogDTO mailLogDTO, @RequestParam(required = false) Map<String, Object> paramMaps, Pageable pageable) throws IllegalAccessException {
        ISearchResponseDTO result = this.mailLogSearchService.simpleQuerySearchWithParam(mailLogDTO, paramMaps, pageable);
        log.debug("MailLogCustomRest: customSearchWithParam(IBaseSearchDTO: {}): {}", mailLogDTO, result);
        return ResponseEntity.ok().body(new LoadedMessage(result));
    }
}
