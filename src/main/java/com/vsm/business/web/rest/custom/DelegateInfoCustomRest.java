...
import com.vsm.business.service.custom.DelegateInfoCustomService;
import com.vsm.business.web.rest.errors.BadRequestAlertException;
import com.vsm.business.service.custom.search.service.DelegateInfoSearchService;
...
@RestController
@RequestMapping("/api")
public class DelegateInfoCustomRest {

    private final Logger log = LoggerFactory.getLogger(DelegateInfoCustomRest.class);
    ...
    private final DelegateInfoCustomService delegateInfoCustomService;

    private final DelegateInfoSearchService delegateInfoSearchService;

    public DelegateInfoCustomRest(DelegateInfoService delegateInfoService, DelegateInfoRepository delegateInfoRepository, DelegateInfoCustomService delegateInfoCustomService, DelegateInfoSearchService delegateInfoSearchService) {
        this.delegateInfoService = delegateInfoService;
        this.delegateInfoRepository = delegateInfoRepository;
        this.delegateInfoCustomService = delegateInfoCustomService;
        this.delegateInfoSearchService = delegateInfoSearchService;
    }
    ...
    @GetMapping("/_all/delegate-infos")
    public ResponseEntity<IResponseMessage> getAll(){
        List<DelegateInfoDTO> result = this.delegateInfoCustomService.getAll();
        log.debug("DelegateInfoCustomRest: getAll(): {}", result);
     ...
     */
    @PostMapping("/search_custom/delegate-infos")
    public ResponseEntity<IResponseMessage> customSearch(@RequestBody DelegateInfoDTO delegateInfoDTO, Pageable pageable) throws IllegalAccessException {
        ISearchResponseDTO result = this.delegateInfoSearchService.simpleQuerySearch(delegateInfoDTO, pageable);
        log.debug("DelegateInfoCustomRest: customSearch(IBaseSearchDTO: {}): {}", delegateInfoDTO, result);
        return ResponseEntity.ok().body(new LoadedMessage(result));
    }

    @PostMapping("/v2/search_custom/delegate-infos")
    public ResponseEntity<IResponseMessage> customSearchWithParam(@RequestBody DelegateInfoDTO delegateInfoDTO, @RequestParam(required = false) Map<String, Object> paramMaps, Pageable pageable) throws IllegalAccessException {
        ISearchResponseDTO result = this.delegateInfoSearchService.simpleQuerySearchWithParam(delegateInfoDTO, paramMaps, pageable);
        log.debug("DelegateInfoCustomRest: customSearchWithParam(IBaseSearchDTO: {}): {}", delegateInfoDTO, result);
        return ResponseEntity.ok().body(new LoadedMessage(result));
    }
}
