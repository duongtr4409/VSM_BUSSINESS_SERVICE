...
import com.vsm.business.service.custom.ChangeFileHistoryCustomService;
import com.vsm.business.web.rest.errors.BadRequestAlertException;
import com.vsm.business.service.custom.search.service.ChangeFileHistorySearchService;
...
@RestController
@RequestMapping("/api")
public class ChangeFileHistoryCustomRest {

    private final Logger log = LoggerFactory.getLogger(ChangeFileHistoryCustomRest.class);
    ...
    private final ChangeFileHistoryCustomService changeFileHistoryCustomService;

    private final ChangeFileHistorySearchService changeFileHistorySearchService;

    public ChangeFileHistoryCustomRest(
        ChangeFileHistoryService changeFileHistoryService,
        ChangeFileHistoryRepository changeFileHistoryRepository,
        ChangeFileHistoryCustomService changeFileHistoryCustomService,
        ChangeFileHistorySearchService changeFileHistorySearchService
    ) {
        this.changeFileHistoryService = changeFileHistoryService;
        this.changeFileHistoryRepository = changeFileHistoryRepository;
        this.changeFileHistoryCustomService = changeFileHistoryCustomService;
        this.changeFileHistorySearchService = changeFileHistorySearchService;
    }
    ...
    @GetMapping("/_all/change-file-histories")
    public ResponseEntity<IResponseMessage> getAll(){
        List<ChangeFileHistoryDTO> result = this.changeFileHistoryCustomService.getAll();
        log.debug("ChangeFileHitoryCustomRest getAll(): {}", result);
        return ResponseEntity.ok().body(new LoadedMessage(result));
    }

    @GetMapping("/attachment-file/{attachmentFileId}/_all/change-file-histories")
    public ResponseEntity<IResponseMessage> getAllByAttachFile(@PathVariable("attachmentFileId") Long attachmentFileId){
        List<ChangeFileHistoryDTO> result = this.changeFileHistoryCustomService.getAllByAttachmentFile(attachmentFileId);
        log.debug("ChangeFileHitoryCustomRest getAllByAttachFile({}): {}", attachmentFileId, result);
        return ResponseEntity.ok().body(new LoadedMessage(result));
    }
     ...
     */
    @PostMapping("/search_custom/change-file-histories")
    public ResponseEntity<IResponseMessage> customSearch(@RequestBody ChangeFileHistoryDTO changeFileHistoryDTO, Pageable pageable) throws IllegalAccessException {
        ISearchResponseDTO result = this.changeFileHistorySearchService.simpleQuerySearch(changeFileHistoryDTO, pageable);
        log.debug("ChangeFileHistoryCustomRest: customSearch(IBaseSearchDTO: {}): {}", changeFileHistoryDTO, result);
        return ResponseEntity.ok().body(new LoadedMessage(result));
    }

    @PostMapping("/v2/search_custom/change-file-histories")
    public ResponseEntity<IResponseMessage> customSearchWithParam(@RequestBody ChangeFileHistoryDTO changeFileHistoryDTO, @RequestParam(required = false) Map<String, Object> paramMaps, Pageable pageable) throws IllegalAccessException {
        ISearchResponseDTO result = this.changeFileHistorySearchService.simpleQuerySearchWithParam(changeFileHistoryDTO, paramMaps, pageable);
        log.debug("ChangeFileHistoryCustomRest: customSearchWithParam(IBaseSearchDTO: {}): {}", changeFileHistoryDTO, result);
        return ResponseEntity.ok().body(new LoadedMessage(result));
    }
}
