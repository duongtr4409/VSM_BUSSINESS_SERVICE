...
import com.vsm.business.service.custom.AttachmentPermisitionCustomService;
import com.vsm.business.web.rest.errors.BadRequestAlertException;
import com.vsm.business.service.custom.search.service.AttachmentPermisitionSearchService;
...
@RestController
@RequestMapping("/api")
public class AttachmentPermisitionCustomRest {

    private final Logger log = LoggerFactory.getLogger(AttachmentPermisitionCustomRest.class);
    ...
    private final AttachmentPermisitionCustomService attachmentPermisitionCustomService;

    private final AttachmentPermisitionSearchService attachmentPermisitionSearchService;

    public AttachmentPermisitionCustomRest(
        AttachmentPermisitionService attachmentPermisitionService,
        AttachmentPermisitionRepository attachmentPermisitionRepository,
        AttachmentPermisitionCustomService attachmentPermisitionCustomService,
        AttachmentPermisitionSearchService attachmentPermisitionSearchService
    ) {
        ...
        this.attachmentPermisitionCustomService = attachmentPermisitionCustomService;
        this.attachmentPermisitionSearchService = attachmentPermisitionSearchService;
    }
    ...
    @GetMapping("/_all/attachment-permisitions")
    public ResponseEntity<IResponseMessage> getAll(){
        List<AttachmentPermisitionDTO> result = this.attachmentPermisitionCustomService.getAll();
        log.debug("AttachmentPermisitionCustomRest: getAll(): {}", result);
        return ResponseEntity.ok().body(new LoadedMessage(result));
    }

    @PostMapping("/_save/attachment-permisitions")
    public ResponseEntity<IResponseMessage> saveAll(@RequestBody List<AttachmentPermisitionDTO> attachmentPermisitionDTOS){
        List<AttachmentPermisitionDTO> result = this.attachmentPermisitionCustomService.saveAll(attachmentPermisitionDTOS);
        log.debug("AttachmentPermisitionCustomRest: saveAll({}): {}", attachmentPermisitionDTOS, result);
        return ResponseEntity.ok().body(new LoadedMessage(result));
    }

    @DeleteMapping("/_delete/attachment-permisitions")
    public ResponseEntity<IResponseMessage> deleteAll(@RequestBody List<AttachmentPermisitionDTO> attachmentPermisitionDTOS){
        List<AttachmentPermisitionDTO> result = this.attachmentPermisitionCustomService.deleteAll(attachmentPermisitionDTOS);
        log.debug("AttachmentPermisitionCustomRest: deleteAll({}): {}", attachmentPermisitionDTOS, result);
        return ResponseEntity.ok().body(new LoadedMessage(result));
    }

    @GetMapping("/user/{userId}/_all/attachment-permisitions")
    public ResponseEntity<IResponseMessage> getAllByUser(@PathVariable("userId") Long userId){
        List<AttachmentPermisitionDTO> result = this.attachmentPermisitionCustomService.getAllByUser(userId);
        log.debug("AttachmentPermisionCustomRest: getAllByUser({}): {}", userId, result);
        return ResponseEntity.ok().body(new LoadedMessage(result));
    }

    @GetMapping("/attachmentFile/{attachmentFileId}/_all/attachment-permisitions")
    public ResponseEntity<IResponseMessage> getAllByAttachmentFile(@PathVariable("attachmentFileId") Long attachmentFileId){
        List<AttachmentPermisitionDTO> result = this.attachmentPermisitionCustomService.getAllByAttachmentId(attachmentFileId);
        log.debug("AttachmentPermisitionCustomRest getAllByAttachmentFile({}): {}", attachmentFileId, result);
        return ResponseEntity.ok().body(new LoadedMessage(result));
    }
     ...
     */
    @PostMapping("/search_custom/attachment-permisitions")
    public ResponseEntity<IResponseMessage> customSearch(@RequestBody AttachmentPermisitionDTO attachmentPermisitionDTO, Pageable pageable) throws IllegalAccessException {
        ISearchResponseDTO result = this.attachmentPermisitionSearchService.simpleQuerySearch(attachmentPermisitionDTO, pageable);
        log.debug("AttachmentPermisitionCustomRest: customSearch(IBaseSearchDTO: {}): {}", attachmentPermisitionDTO, result);
        return ResponseEntity.ok().body(new LoadedMessage(result));
    }

    @PostMapping("/v2/search_custom/attachment-permisitions")
    public ResponseEntity<IResponseMessage> customSearchWithParam(@RequestBody AttachmentPermisitionDTO attachmentPermisitionDTO, @RequestParam(required = false) Map<String, Object> paramMaps, Pageable pageable) throws IllegalAccessException {
        ISearchResponseDTO result = this.attachmentPermisitionSearchService.simpleQuerySearchWithParam(attachmentPermisitionDTO, paramMaps, pageable);
        log.debug("AttachmentPermisitionCustomRest: customSearchWithParam(IBaseSearchDTO: {}): {}", attachmentPermisitionDTO, result);
        return ResponseEntity.ok().body(new LoadedMessage(result));
    }
