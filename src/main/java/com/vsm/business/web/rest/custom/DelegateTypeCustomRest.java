...
import com.vsm.business.service.custom.DelegateTypeCustomService;
import com.vsm.business.web.rest.errors.BadRequestAlertException;
import com.vsm.business.service.custom.search.service.DelegateTypeSearchService;
...
@RestController
@RequestMapping("/api")
public class DelegateTypeCustomRest {

    private final Logger log = LoggerFactory.getLogger(DelegateTypeCustomRest.class);
    ...
    private final DelegateTypeCustomService delegateTypeCustomService;

    private final DelegateTypeSearchService delegateTypeSearchService;

    public DelegateTypeCustomRest(DelegateTypeService delegateTypeService, DelegateTypeRepository delegateTypeRepository, DelegateTypeCustomService delegateTypeCustomService, DelegateTypeSearchService delegateTypeSearchService) {
        this.delegateTypeService = delegateTypeService;
        this.delegateTypeRepository = delegateTypeRepository;
        this.delegateTypeCustomService = delegateTypeCustomService;
        this.delegateTypeSearchService = delegateTypeSearchService;
    }
    ...
    @GetMapping("/_all/delegate-types")
    public ResponseEntity<IResponseMessage> getAll(){
        log.info("DelegateTypeCustomRest: getAll(): ");
        List<DelegateTypeDTO> result = this.delegateTypeCustomService.getAll();
     ...
     */
    @PostMapping("/search_custom/delegate-types")
    public ResponseEntity<IResponseMessage> customSearch(@RequestBody DelegateTypeDTO delegateTypeDTO, Pageable pageable) throws IllegalAccessException {
        ISearchResponseDTO result = this.delegateTypeSearchService.simpleQuerySearch(delegateTypeDTO, pageable);
        log.debug("DelegateTypeCustomRest: customSearch(IBaseSearchDTO: {}): {}", delegateTypeDTO, result);
        return ResponseEntity.ok().body(new LoadedMessage(result));
    }

    @PostMapping("/v2/search_custom/delegate-types")
    public ResponseEntity<IResponseMessage> customSearchWithParam(@RequestBody DelegateTypeDTO delegateTypeDTO, @RequestParam(required = false) Map<String, Object> paramMaps, Pageable pageable) throws IllegalAccessException {
        ISearchResponseDTO result = this.delegateTypeSearchService.simpleQuerySearchWithParam(delegateTypeDTO, paramMaps, pageable);
        log.debug("DelegateTypeCustomRest: customSearchWithParam(IBaseSearchDTO: {}): {}", delegateTypeDTO, result);
        return ResponseEntity.ok().body(new LoadedMessage(result));
    }
}
