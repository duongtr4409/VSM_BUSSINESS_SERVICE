...
//        org.docx4j.Docx4jProperties.setProperty("com.plutext.converter.URL", "http://127.0.0.1:9016/v1/00000000-0000-0000-0000-000000000000/convert");
    ...
    public InputStream convertDocToPDFDOCX4JDuowngTora(InputStream inputStreamSource) throws IOException, Docx4JException, DocumentException {
        String tempFolderPath = (tempFolder + String.valueOf(System.currentTimeMillis()) + PATH_SEPARATOR).replace("//", PATH_SEPARATOR);
        File folderTemp = new File(tempFolderPath);
        if(!folderTemp.exists()) folderTemp.mkdir();
        ...
        try {
            ...
            FileInputStream fileInputStream = new FileInputStream(fileTempPath);
            XWPFDocument document = new XWPFDocument(fileInputStream);
            PdfOptions options = PdfOptions.create();
            PdfConverter.getInstance().convert(document, fileResultOutputStream, options);
            fileResultOutputStream.flush();
            fileResultOutputStream.close();
            fileInputStream.close();
            return new ByteArrayInputStream(org.apache.commons.io.FileUtils.readFileToByteArray(resultFileTemp));
        }finally {
            try { sourceFileTemp.deleteOnExit(); }catch (Exception e){log.error("{}", e);}
            try { resultFileTemp.deleteOnExit(); }catch (Exception e){log.error("{}", e);}
            try { folderTemp.deleteOnExit(); }catch (Exception e){log.error("{}", e);}
        }

    }

    // hàm này convert được file sang pdf nhưng bị xấu
    public InputStream convertDocToPDFDOCX4JDuowngTora_bak(InputStream inputStreamSource) throws IOException, Docx4JException, DocumentException {
        String tempFolderPath = (tempFolder + String.valueOf(System.currentTimeMillis()) + PATH_SEPARATOR).replace("//", PATH_SEPARATOR);
        File folderTemp = new File(tempFolderPath);
        if(!folderTemp.exists()) folderTemp.mkdir();
